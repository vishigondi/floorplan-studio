...
function draw(L) {
  ...
  const gap = L.units.length > 1 ? `min gap ${minGapFt(L)} ft` : 'single unit';
  ...
  <figcaption><b>${L.title}</b><span>${L.unitCount} unit(s) &middot; needs ${L.requiresEntry.toUpperCase()} entry &middot; ${gap}</span>
  <span class="${layoutTowsClear(L) ? 'ok' : 'no'}">${tow}</span></figcaption>
</figure>`;
}
...
</style><h1>Site layouts — dark = unit, tan = open deck, dashed = tow sweep (green clear / red blocked). Uphill is up.</h1>
